document.addEventListener('DOMContentLoaded', function() {
    ...
    function addFoodOptions(items) {
        ...
        card1.querySelector('img').src = items[0].image_url;
        card1.querySelector('h3').textContent = items[0].item_name;
        card1.querySelector('p em').textContent = items[0].restaurant_name;
        card1.querySelector('.price').textContent = items[0].price;

        // Set food item 2 details
        const card2 = foodOptions.querySelector('.food-card:last-child');
        card2.querySelector('img').src = items[1].image_url;
        card2.querySelector('h3').textContent = items[1].item_name;
        card2.querySelector('p em').textContent = items[1].restaurant_name;
        card2.querySelector('.price').textContent = items[1].price;
        ...
        card1.querySelector('button').addEventListener('click', () => {
            socket.emit('order', {
                ...
                restaurant_url: items[0].restaurant_url
            });
        });
        ...
        card2.querySelector('button').addEventListener('click', () => {
            socket.emit('order', {
                ...
                restaurant_url: items[1].restaurant_url
            });
        });
    ...
    socket.on('reminder', function(data) {
        ...
        if (data.food_options) {
            setTimeout(()=>addFoodOptions(data.food_options), 500)
        }
    });
